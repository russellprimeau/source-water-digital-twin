...
The model spread driving the sampling plan is computed from an ensemble of
model alternatives, and the sensitivity of the result to the number of those
alternatives is reported by src/model/BuildSamplingPriority.py.  What this script
establishes is a different thing: how sensitive the *downstream* result -- the
clustering and the selected route -- is to the particular set of cells that
entered it.
...
Writes data/sampling_plan/sampling_plan_stability.csv and prints a summary.

No script in this repository reaches the network: shorelines are drawn from the
published mesh boundary rather than from downloaded map tiles.
"""
...
# Public header for the model-spread score used by the planning analysis.
# Section 2.3: the layer mean of the across-member standard deviation divided by
# the across-member mean, at cells whose predicted concentration clears a
# model-signal threshold. It is not an uncertainty estimate.
SENSITIVITY_COLUMN = "Model-spread score"
# The generated text is LaTeX, so a literal percent sign must be escaped.
PCT = chr(92) + "%"


def pct(count, total, places=1):
    """Percentage of an exact ratio, rounded half-up, as a string.

    Binary floating point breaks ties such as 1267/2000 = 63.35% either way
    depending on how the expression is written, which let one quantity be
    reported as both 63.3% and 63.4%.
    """
    value = Decimal(count) * 100 / Decimal(total)
    return str(value.quantize(Decimal(1).scaleb(-places), rounding=ROUND_HALF_UP))
# Shared with a.PointSelection.py through route_solver, so the radius the article
# reports and the radius this analysis resamples under cannot differ.
EPS_METRES = route_solver.CLUSTER_EPS_M
EARTH_RADIUS_M = route_solver.EARTH_RADIUS_M
# Shared with b.PathOptimization2.py; see route_solver for the value and why it is
# a mission-duration limit rather than an endurance limit.
MAX_PATH_LENGTH_M = route_solver.MAX_PATH_LENGTH_M
LAUNCH_POINT = np.array(route_solver.LAUNCH_POINT)  # Vasstrandlia ramp
N_BOOTSTRAP = 2_000
...
def best_route(clusters: pd.DataFrame) -> tuple[tuple[int, ...], float, float]:
    ...
    Maximise summed weight subject to the path-length constraint, breaking ties
    on shorter distance.  Returned as the tuple of visited cluster indices.
    Shares route_solver.solve() with b.PathOptimization2.py, so the baseline here
    and the published route are computed by the same code.
    """
    ...
    best_order, best_score, best_len = route_solver.solve(dist, weights,
                                                          MAX_PATH_LENGTH_M)
...
def main() -> None:
    ...
    print(f"  same cluster count : {loo_same_count}/{n_cells} ({pct(loo_same_count, n_cells)}%)")
    print(f"  same selected route: {loo_same_route}/{n_cells} ({pct(loo_same_route, n_cells)}%)")
    ...
    for k in sorted(boot_ncluster):
        print(f"    {k} clusters: {pct(boot_ncluster[k], N_BOOTSTRAP):>5}%")
    ...
    for route, count in boot_routes.most_common(5):
        ...
        print(f"    {str(route):24s} {pct(count, N_BOOTSTRAP):>5}%{marker}")
    ...
    # When the baseline set is also the modal set, report that one percentage
    # once. Computing it twice by different routes, 100*a/b and 100*(a/b),
    # rounded the same quantity to two different figures.
    baseline_pct = pct(boot_routes[base_route], N_BOOTSTRAP)
    if summary['modal_regions'] == summary['baseline_regions']:
        bootstrap_sentence = (
            "the baseline region set "
            + '--'.join(summary['baseline_regions'])
            + " was also the most frequent, recurring in "
            + baseline_pct + PCT + " of replicates.")
    else:
        bootstrap_sentence = (
            "its selected region set recurred in "
            + baseline_pct + PCT + " of cases. The most frequent "
            "region set was " + '--'.join(summary['modal_regions'])
            + ", occurring in "
            + pct(boot_routes.most_common(1)[0][1], N_BOOTSTRAP) + PCT
            + " of replicates.")
    # How many clusters hold more than one cell, and so the budget, are properties of
    # the run. Spelling either into the sentence makes the snippet silently wrong the
    # next time the inputs change.
    vals = out['mean_co_association'].dropna()
    words = {1: 'one', 2: 'two', 3: 'three', 4: 'four', 5: 'five', 6: 'six',
             7: 'seven', 8: 'eight', 9: 'nine', 10: 'ten'}
    if len(vals) == 1:
        co_sentence = (f"Mean pairwise co-association within the single baseline cluster "
                       f"holding more than one cell is {vals.iloc[0]:.3f}, conditional on "
                       "both cells being sampled. ")
    else:
        co_sentence = (f"Mean pairwise co-association within the "
                       f"{words.get(len(vals), len(vals))} multi-cell baseline clusters "
                       f"ranged from {vals.min():.3f} to {vals.max():.3f}, conditional on "
                       "both cells being sampled. ")
    manuscript = (
    ...
    out = ROOT_DIR / 'output' / 'sampling_stability_results.tex'
